...
import { PropertyItem, GoogleReviewItem, SiteSettings, CounterItem, LocationNode, ProjectCategory, LeadershipProfile, VideoTestimonialItem } from '../types';
...
export const DEFAULT_LEADERSHIP: LeadershipProfile[] = [
  {
    ...
    officeLocation: 'G-102, Nerul Railway Station Complex, Navi Mumbai',
    experience: '25+ Years Real Estate Industry Leadership',
    badges: ['Founder', 'CIDCO Title Specialist', 'Navi Mumbai Expert']
  },
  {
    id: 'lead-ceo',
    role: 'CEO',
    ...
    officeLocation: 'Head Office, Nerul Railway Station Complex, Navi Mumbai',
    experience: '18+ Years Strategic Advisory & Client Experience',
    badges: ['CEO', 'Customer Experience Pioneer', 'Airport Node Specialist']
  }
];

export const DEFAULT_VIDEO_TESTIMONIALS: VideoTestimonialItem[] = [
  {
    id: 'v1',
...
const DEFAULT_SITE_SETTINGS: SiteSettings = {
  ...
  smtpPort: '465',
  smtpUser: 'Jayshreerealty16@gmail.com',
  smtpFromEmail: 'Jayshreerealty16@gmail.com',
...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [leadership, setLeadership] = useState<LeadershipProfile[]>(DEFAULT_LEADERSHIP);
  ...
  const refreshData = async () => {
    try {
      ...
      // 3. Fetch Leadership Profiles
      const leaderRes = await api.getLeadership();
      if (leaderRes.success && Array.isArray(leaderRes.leadership) && leaderRes.leadership.length > 0) {
        setLeadership(leaderRes.leadership);
      }

      // 4. Fetch Video Testimonials
      const videoRes = await api.getVideoTestimonials();
      if (videoRes.success && Array.isArray(videoRes.videos) && videoRes.videos.length > 0) {
        setVideoTestimonials(videoRes.videos.map((v: any) => ({
          id: v.id,
  ...
  const addReview = async (reviewData: Omit<GoogleReviewItem, 'id'>) => {
    ...
    if (res.success && res.review) {
      setReviews(prev => [{ ...reviewData, id: res.review.id } as GoogleReviewItem, ...prev]);
    }
  };
